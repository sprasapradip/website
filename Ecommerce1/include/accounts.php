...
class User {
	protected static  $tname = "tuseraccount";

	function dbfields () {
		global $mydb;
		return $mydb->getfieldsononetable(self::$tname);
	}
	function listofuser(){
		global $mydb;
		$mydb->setQuery("SELECT * FROM ".self::$tname);
		return $cur;
	}
	...
	static function userAuthentication($U_USERNAME,$h_pass){
		global $mydb;
		$mydb->setQuery("SELECT * FROM `tuseraccount` WHERE `U_USERNAME` = '". $U_USERNAME ."' and `U_PASS` = '". $h_pass ."'");
		$cur = $mydb->executeQuery();
		if($cur==false){
			die(mysql_error());
		}
